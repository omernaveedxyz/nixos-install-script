...
let
  ...
  installedConfig = {
    ...
    boot.initrd.luks.devices."hibernate".device = "/dev/disk/by-label/hibernate-luks";
...
in
{
  name = "Hibernation Configuration Test p.2";
  ...
  testScript = ''
    ...
    install_configuration("echo y | ${install-script}/bin/install.sh --hostname=hibernate --luks=supersecretpassword --hibernation /dev/vda")
    machine.shutdown()

    # verify that client boots
    hibernate = create_named_machine("hibernate")
    hibernate.start()
    hibernate.wait_for_console_text("Starting password query on")
    hibernate.send_console("supersecretpassword\n")
    ...
    resume.start()
    resume.wait_for_console_text("Starting password query on")
    resume.send_console("supersecretpassword\n")
